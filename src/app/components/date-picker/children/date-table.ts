...
@Component({
  selector: 'ql-date-table',
  providers: [DateFormat],
  template: `
    <table class="ql-date-table" cellspacing="0" cellpadding="0">
      <tbody>
      <tr>
        <th *ngFor="let week of weeks">{{week}}</th>
      </tr>
      <tr class="ql-date-table__row"
...
export class QlDateTable implements OnInit, OnChanges {
  ...
  getRows(): void {
    ...
    this.tableRows = this.tableRows.map((row, index) => {
      if (index === 0) {
        return QlDateTable.BuildMonthStartRow(firstDay, lastMonthDayCount)
      }
