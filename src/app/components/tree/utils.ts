...
export class ModelStandard {
  ...
  static LoopRemoveChecked(models: QlTreeModelData[]): void {
    models.forEach(item => {
      item.checked = false
  ...
  static DeepUpdateExpanded(id: string | number, models: QlTreeModelData[], accordion: boolean): QlTreeModelData[] {
  ...
  static DeepUpdateChecked(id: string | number, models: QlTreeModelData[]): QlTreeModelData[] {
    if (!models || !models.length) return []
    const index = models.findIndex(item => item.id === id)

    if (index === -1) {
      models.forEach(item => {
        const nextChildren: QlTreeModelData[] = ModelStandard.DeepUpdateChecked(id, item.children || [])
  ...
  static SetChildrenChecked(models: QlTreeModelData[], checked: boolean): void {
    models.forEach(item => {
  ...
  static findWholeModel(models: QlTreeModelData[]): any[] {
    return models
  }

  static FindAllChecked(models: QlTreeModelData[]): string[] {
  ...
  filterModel(models: QlTreeModelData[]): QlTreeModelData[] {
    return this.updateDepthIdent(models, this.init.initDepth)
  }

  private updateDepthIdent(models: QlTreeModelData[], depth: number)
    : QlTreeModelData[] {
    return models.map(item => {
      const nextID: string | number = item.id || makeRandomID()
      const nextChildren: QlTreeModelData[] = notEmptyArray(item.children)
  ...
  private isExpanded(id: string | number, item: QlTreeModelData): boolean {
  ...
  private isChecked(id: string | number, item: QlTreeModelData): boolean {
