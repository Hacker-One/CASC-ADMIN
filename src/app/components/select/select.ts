...
import { QlSelectPoprs } from './select-props'
...
@Component({
  ...
  providers: [{
    ...
    useExisting: forwardRef(() => QlSelect),
    multi: true
  }],
  styles: [`
    .ql-select-dropdown__list { overflow: hidden; }
    .ql-select__tags__padding { padding-right: 30px; }
  `],
  template: `
    <div class="ql-select" (click)="toggleHandle($event)">
      <div class="ql-select__tags ql-select__tags__padding" *ngIf="multiple && model && model.length" #tags>
        <ql-tag *ngFor="let tag of multipleLabels; let i = index"
          [closable]="!qlDisabled"
          [size]="size"
          (close)="$event.stopPropagation();changeLabel(tag, model[i])"
          type="info">{{tag}}</ql-tag>
      </div>
      
      <ql-input iconClass="ql-select__caret" #input
        [model]="selectedLabel"
        [placeholder]="multiple ? multiplePlaceholder : placeholder"
        [icon]="iconClass"
        [name]="name"
        [size]="size"
        [qlDisabled]="qlDisabled" [readonly]="true"
        (mouseenter)="mouseHandle(true)" (mouseleave)="mouseHandle(false)"
        (icon-click)="clearSelected($event)">
      </ql-input>
      <ng-container>
        <ql-select-dropdown [isActived]="dropdownActive">
          <ul class="ql-scrollbar__view ql-select-dropdown__list">
...
export class QlSelect extends QlSelectPoprs implements OnInit, OnDestroy, OnChanges, AfterViewInit, ControlValueAccessor {
  ...
  private updateLayoutWithMultipleMode(): void {
    const updateHandle = () => {
      ...
      const el = inputEl.querySelector('.ql-input__inner')
