...
export class QlLoadingDirective implements OnInit {
  ...
  @Input('ql-loading-top') elLoadingTop: string = '50%'
  @Input('ql-loading') set showLoading(val: boolean) {
    this.visible = val
  ...
  makeHtml(): string {
    ...
    return `
      <div class="ql-loading-mask ${this.customClass} ${this.fullScreen ? ' is-fullscreen' : ''}"
        style="display: ${this.visible ? 'block': 'none'}">
        <div class="ql-loading-spinner" style="top: ${this.elLoadingTop}">
          <svg class="circular" viewBox="25 25 50 50">
            <circle class="path" cx="50" cy="50" r="20" fill="none"/>
          </svg>
          <p class="ql-loading-text" style="display: ${this.text ? 'block': 'none'}">
            ${this.text}
          </p>
        </div>
      </div>
